...
Following is the example command to deploy the application in GCP Compute Engine: (DO NOT ADD ADDITIONAL PARAMETERS LIKE NETWORK, SUBNET, ETC.,)
gcloud compute instances create <VM name> --zone=us-east1-c --machine-type=e2-micro --scopes=cloud-platform --tags=http-server,web-server --metadata-from-file=startup-script=<path to .sh file created>
...
echo "Updating package list and installing required packages..."
sudo apt-get update -y
sudo apt-get install -y python3 python3-pip git python3-venv gunicorn ufw (MAKE SURE TO INSTALL to install all the required dependencies that are required to run the application and shell script like ufw, venv, gunicorn etc.,)
echo "Configuring firewall to allow traffic on port $APP_PORT..."
sudo ufw allow $APP_PORT
sudo ufw --force enable
echo "Setting up application directory at $APP_DIR..."
